...
```bash
gog drive (drv) comments reply (respond) <fileId> <commentId> <content> [flags]
```
...
| `--action` | `string` |  | Optional action to take on the parent comment alongside the reply: resolve\|reopen |
